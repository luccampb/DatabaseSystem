...
public class GigTester {
    ...
    public static boolean testTask2(){
        ...
        try {
            String[][] out = GigSystem.task1(GigSystem.getSocketConnection(),51);
            if(out.length != apd.length) {
                throw new TestFailedException("Length " + out.length,"Length " + apd.length);
            }
        } catch (Exception e) {
            e.printStackTrace();
            return false;
    ...
    public static boolean testTask3(){
        ...
        //GigSystem.task3(GigSystem.getSocketConnection(), gigid, name, email,"B");
        //GigSystem.task3(GigSystem.getSocketConnection(), 52, name, email,"A");
    ...
    public static boolean testTask4(){
        ...
        String[][] expectedEmails = {{"c22cc@example.com","d33dd@example.com"}};
        System.out.println("Test1");
        //In the case the gig is not cancelled, you can do something similar to testTask1
        try {
            for(int i = 0; i < out[0].length; i++){
                checkValues(out[0][i],expectedEmails[0][i]);
            }            
        } catch (Exception e) {
            e.printStackTrace();
            return false;
        ...
        // cancelGigID = 1;
        // actName = "Tervis Scoot";
        // out = GigSystem.task4(GigSystem.getSocketConnection(),cancelGigID,actName);

        // //You should put some code here to read the state of the database after calling task 4 to confirm that the input gig has been cancelled

        // //For instance, if gigid 40 was to be cancelled in the main test data, the string should be:
        // String[][] expected = {{"Small Submachinegun Green", "18:00:00", "19:00:00"},{"Playman Cartier", "20:10:00", "20:40:00"}};
        // System.out.println(out[0].length);
        // //In the case the gig is not cancelled, you can do something similar to testTask1
        // try {
        //     for(int i = 0; i < out[0].length; i++){
        //         checkValues(out[0][i],expected[0][i]);
        //         System.out.println(out[0][i]);
        //         System.out.println(expected[0][i]);
        //     }            
        // } catch (Exception e) {
        //     e.printStackTrace();
        //     return false;
